...
- **macOS:** `brew install awscli` (or download from AWS CLI Install page)
...
> These credentials are temporary and will expire. If you get authentication errors later, go back to Workshop Studio and copy fresh credentials.
...
| Terminal | Use the integrated terminal to run commands |
...
Once the MCP server is connected, you can ask Kiro to query it:
...
- [Full-Stack Solution Template for AgentCore](https://github.com/awslabs/fullstack-solution-template-for-agentcore)

---

## Step 6 — Build AI Agents with Strands SDK (Optional)

The **Strands Agents SDK** is an open-source Python SDK for building agents that can use tools, call LLMs, and orchestrate multi-step reasoning.
...
1. **Use Kiro.** It knows the AWS stack.
2. **Plan first.** Use Kiro's Planner (`Shift + Tab`) to break down your idea into steps before coding.
3. **Start simple.** Get a basic CRUD app working first, then add features.
4. **Check the Amplify docs.** https://docs.amplify.aws/gen2/
